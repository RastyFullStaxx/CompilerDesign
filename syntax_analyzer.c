...
ParseTreeNode* parseProgram() {
    ...
    // Track the last processed token for loop safety
    ...
    while (peekToken()) {
        ...
        if (lastToken == currentToken) {
            loopSafetyCounter++;
            if (loopSafetyCounter > 100) { // Infinite loop protection
            ...
            loopSafetyCounter = 0; // Reset loop safety counter if token changes
        }
        ...
        if (!statementNode) {
            ...
            // Error recovery: Attempt to skip invalid tokens
            if (!recoverFromError()) {
                printf("[ERROR] Unable to recover from syntax error. Ending parsing.\n");
                break;
            }

            continue; // Continue parsing after recovery
        }

        addChild(root, statementNode);

        // Debug the addition of the parsed statement to the root
...
ParseTreeNode* parseStatement() {
    ...
    // Debug: Print parent-child relationship
    printf("[DEBUG] Created Node for Statement: Label='%s', Value='%s'\n",
           statementNode->label, statementNode->value);
...
ParseTreeNode* parseForLoop() {
    ...
    ParseTreeNode* initStatement = parseDeclarationStatement();
    if (!initStatement) {
        reportSyntaxError("Failed to parse initialization statement in for loop.");
        recoverFromError();
        freeParseTree(forNode);
        return NULL;
    }
    addChild(forNode, initStatement);
    ...
    // Parse condition
    ParseTreeNode* condition = parseExpression();
    if (!condition) {
        reportSyntaxError("Failed to parse condition in for loop.");
        recoverFromError();
        freeParseTree(forNode);
        return NULL;
    }
    ...
    // Match ';'
    if (!matchToken("Delimiter", ";")) {
        reportSyntaxError("Expected ';' after for-condition.");
        recoverFromError();
        freeParseTree(forNode);
        return NULL;
    }
    ...
    ParseTreeNode* incrementExpr = parseExpression();
    if (!incrementExpr) {
        reportSyntaxError("Failed to parse increment expression in for loop.");
        recoverFromError();
        freeParseTree(forNode);
        return NULL;
    }
    addChild(forNode, incrementExpr);

    // Match ')'
    if (!matchToken("Delimiter", ")")) {
        reportSyntaxError("Expected ')' after for increment expression.");
        recoverFromError();
        freeParseTree(forNode);
        return NULL;
    }
    ...
    ParseTreeNode* statementBlock = parseStatementBlock();
    if (!statementBlock) {
        reportSyntaxError("Failed to parse statement block in for loop.");
        recoverFromError();
        freeParseTree(forNode);
        return NULL;
    }
    addChild(forNode, statementBlock);
...
ParseTreeNode* parseAssignmentExpr() {
    ...
    ParseTreeNode* leftOperand = parsePrimaryExpr(); // Usually an identifier or primary expression
    if (!leftOperand) {
    ...
    // Match assignment operator if present
    if (token && strcmp(token->type, "AssignmentOperator") == 0) {
        ...
        // Parse the right-hand operand (higher precedence: relational or additive)
        ParseTreeNode* rightOperand = parseRelationalExpr(); // Call relational expression as it's higher precedence
        if (!rightOperand) {
